...
public class TarefaConcluidaFragment extends ListFragment {
	 ...
	 */
	public static TarefaConcluidaFragment novaInstancia(
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		case R.id.deConcluidaParaProcesso:
			...
			for (int i = 0; i < listaTarefaConcluida.size(); i++) {
				if (listaTarefaConcluida.get(i).getTarefa().getCodigo() == tarefaSelecionada
						.getTarefa().getCodigo()) {
					listaTarefaConcluida.remove(i);
				}
			}
			((MudarParaProcesso) getActivity())
					.clicouTarefaVoltarProcesso(tarefaSelecionada);
			AtualizarListaDeTarefa();
			break;
		}
