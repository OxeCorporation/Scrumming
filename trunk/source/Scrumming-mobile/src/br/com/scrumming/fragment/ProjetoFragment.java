...
public class ProjetoFragment extends ListFragment {
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		if (listaProjetos != null) {
			...
			if (taskProjeto != null
					&& taskProjeto.getStatus() == Status.RUNNING) {
				mostrarProgress();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View layout = inflater.inflate(R.layout.fragment_projetos, container,
				false);

		progressProjeto = (ProgressBar) layout
				.findViewById(R.id.progressBarProjeto);
		txtMensagemProjeto = (TextView) layout
				.findViewById(R.id.txtMensagemProjeto);
	...
	public void onListItemClick(ListView l, View v, int position, long id) {
		...
		if (getActivity() instanceof ClickedOnProjeto) {
			((ClickedOnProjeto) getActivity()).projetoFoiClicado(
					listaProjetos.get(position), usuarioEmpresa);
		}
	}
	...
	private void AtualizarListaDeProjetos() {
		ProjetoAdapter adapter = new ProjetoAdapter(getActivity(),
				listaProjetos);
		setListAdapter(adapter);
		if (listaProjetos.size() == 1) {
			if (getActivity() instanceof ClickedOnProjeto) {
				((ClickedOnProjeto) getActivity()).projetoFoiClicado(
						listaProjetos.get(0), usuarioEmpresa);
			}
		}
	}

	// InnerClass do AsyncTask da Empresa
	class AsyncTaskProjeto extends
			AsyncTask<UsuarioEmpresa, Void, List<Projeto>> {
